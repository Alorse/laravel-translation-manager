...
class Controller extends BaseController
{
    ...
    {
        $this->package = \Alorse\TranslationManager\ManagerServiceProvider::PACKAGE;
    ...
    protected function loadLocales()
    {
        ...
        $locales = array_merge([$primaryLocale, $translatingLocale, $currentLocale], $configLocales, $locales);
    ...
    {
        Route::get('view/{group?}', '\\Alorse\\TranslationManager\\Controller@getView');

        //deprecated: Route::controller('admin/translations', '\\Alorse\\TranslationManager\\Controller');
        Route::get('/', '\\Alorse\\TranslationManager\\Controller@getIndex');
        Route::get('connection', '\\Alorse\\TranslationManager\\Controller@getConnection');
        Route::get('import', '\\Alorse\\TranslationManager\\Controller@getImport');
        Route::get('index', '\\Alorse\\TranslationManager\\Controller@getIndex');
        Route::get('interface_locale', '\\Alorse\\TranslationManager\\Controller@getInterfaceLocale');
        Route::get('keyop/{group}/{op?}', '\\Alorse\\TranslationManager\\Controller@getKeyop');
        Route::get('publish/{group}', '\\Alorse\\TranslationManager\\Controller@getPublish');
        Route::get('search', '\\Alorse\\TranslationManager\\Controller@getSearch');
        Route::get('toggle_in_place_edit', '\\Alorse\\TranslationManager\\Controller@getToggleInPlaceEdit');
        Route::get('trans_filters', '\\Alorse\\TranslationManager\\Controller@getTransFilters');
        Route::get('translation', '\\Alorse\\TranslationManager\\Controller@getTranslation');
        Route::get('usage_info', '\\Alorse\\TranslationManager\\Controller@getUsageInfo');
        Route::get('view', '\\Alorse\\TranslationManager\\Controller@getView');
        Route::get('zipped_translations/{group?}', '\\Alorse\\TranslationManager\\Controller@getZippedTranslations');
        Route::post('add/{group}', '\\Alorse\\TranslationManager\\Controller@postAdd');
        Route::post('copy_keys/{group}', '\\Alorse\\TranslationManager\\Controller@postCopyKeys');
        Route::post('delete/{group}/{key}', '\\Alorse\\TranslationManager\\Controller@postDelete');
        Route::post('delete_all/{group}', '\\Alorse\\TranslationManager\\Controller@postDeleteAll');
        Route::post('delete_keys/{group}', '\\Alorse\\TranslationManager\\Controller@postDeleteKeys');
        Route::post('delete_suffixed_keys{group?}', '\\Alorse\\TranslationManager\\Controller@postDeleteSuffixedKeys');
        Route::post('edit/{group}', '\\Alorse\\TranslationManager\\Controller@postEdit');
        Route::post('find', '\\Alorse\\TranslationManager\\Controller@postFind');
        Route::post('import/{group}', '\\Alorse\\TranslationManager\\Controller@postImport');
        Route::post('move_keys/{group}', '\\Alorse\\TranslationManager\\Controller@postMoveKeys');
        Route::post('preview_keys/{group}', '\\Alorse\\TranslationManager\\Controller@postPreviewKeys');
        Route::post('publish/{group}', '\\Alorse\\TranslationManager\\Controller@postPublish');
        Route::post('show_source/{group}/{key}', '\\Alorse\\TranslationManager\\Controller@postShowSource');
        Route::post('undelete/{group}/{key}', '\\Alorse\\TranslationManager\\Controller@postUndelete');
        Route::post('user_locales', '\\Alorse\\TranslationManager\\Controller@postUserLocales');
        Route::post('yandex_key', '\\Alorse\\TranslationManager\\Controller@postYandexKey');
    }
    ...
    private function processIndex($group = null)
    {
        ...
        $groups = ['' => noEditTrans($this->packagePrefix . 'messages.choose-group')] + $this->manager->getGroupList();
    ...
    public function postDeleteSuffixedKeys($group)
    {
        if (Gate::allows(Manager::ABILITY_ADMIN_TRANSLATIONS)) {
            if (!in_array($group, $this->manager->config(Manager::EXCLUDE_GROUPS_KEY)) && $this->manager->config('admin_enabled')) {
                ...
                if (count($suffixes) === 1 && $suffixes[0] === '') {
                    $suffixes = [];
                }
    ...
    public function isLocaleEnabled($locale)
    {
        ...
        foreach ($locale as $item) {
            if (!str_contains($this->userLocales, ',' . $item . ',')) {
                return false;
            }
        }
        return true;
    ...
    public function postShowSource($group, $key)
    {
        ...
        sort($refs);
        return ['status' => 'ok', 'result' => $refs, 'key_name' => "$group.$key"];
    }
    ...
    protected function keyOp($group, $op = 'preview')
    {
        ...
        if ($userLocales) {
            $userLocales = "'" . str_replace(',', "','", substr($userLocales, 1, -1)) . "'";
        }
    ...
    public function getImport()
    {
        ...
        return Response::json(['status' => 'ok', 'counter' => $counter, 'errors' => $errors]);
    }
    ...
    public function postDeleteAll($group)
    {
        ...
        return Response::json(['status' => 'ok', 'counter' => (int)0]);
    }
    ...
    public function getPublish($group)
    {
        ...
        return Response::json(['status' => $errors ? 'errors' : 'ok', 'errors' => $errors]);
    }
